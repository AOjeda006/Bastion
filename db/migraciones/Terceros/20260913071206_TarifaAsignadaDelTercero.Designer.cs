// <auto-generated />
using System;
using System.Collections.Generic;
using Bastion.Terceros.Infrastructure.Persistencia;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using Npgsql.EntityFrameworkCore.PostgreSQL.Metadata;

#nullable disable

namespace Bastion.Terceros.Infrastructure.Migrations
{
    [DbContext(typeof(TercerosDbContext))]
    [Migration("20260913071206_TarifaAsignadaDelTercero")]
    partial class TarifaAsignadaDelTercero
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasDefaultSchema("terceros")
                .HasAnnotation("ProductVersion", "10.0.9")
                .HasAnnotation("Relational:MaxIdentifierLength", 63);

            NpgsqlModelBuilderExtensions.UseIdentityByDefaultColumns(modelBuilder);

            modelBuilder.Entity("Bastion.BuildingBlocks.Infrastructure.Auditoria.RegistroDeAuditoria", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<string>("Cambio")
                        .IsRequired()
                        .HasMaxLength(32)
                        .HasColumnType("character varying(32)")
                        .HasColumnName("cambio");

                    b.Property<Guid>("CorrelacionId")
                        .HasColumnType("uuid")
                        .HasColumnName("correlacion_id");

                    b.Property<Guid?>("EmpresaId")
                        .HasColumnType("uuid")
                        .HasColumnName("empresa_id");

                    b.Property<string>("Entidad")
                        .IsRequired()
                        .HasMaxLength(128)
                        .HasColumnType("character varying(128)")
                        .HasColumnName("entidad");

                    b.Property<string>("EntidadId")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("character varying(256)")
                        .HasColumnName("entidad_id");

                    b.Property<DateTimeOffset>("OcurridoEn")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("ocurrido_en");

                    b.Property<string>("SinInquilino")
                        .HasMaxLength(64)
                        .HasColumnType("character varying(64)")
                        .HasColumnName("sin_inquilino");

                    b.Property<Guid?>("UsuarioId")
                        .HasColumnType("uuid")
                        .HasColumnName("usuario_id");

                    b.Property<string>("Valores")
                        .IsRequired()
                        .HasColumnType("jsonb")
                        .HasColumnName("valores");

                    b.HasKey("Id")
                        .HasName("pk_registros");

                    b.HasIndex("CorrelacionId")
                        .HasDatabaseName("ix_registros_correlacion_id");

                    b.HasIndex("OcurridoEn")
                        .HasDatabaseName("ix_registros_ocurrido_en");

                    b.HasIndex("Entidad", "EntidadId")
                        .HasDatabaseName("ix_registros_entidad_entidad_id");

                    b.ToTable("registros", "auditoria", t =>
                        {
                            t.ExcludeFromMigrations();

                            t.HasCheckConstraint("ck_registros_empresa_o_motivo", "(empresa_id IS NULL) <> (sin_inquilino IS NULL)");
                        });

                    b.HasAnnotation("Bastion:Auditoria", "NoAuditada|es la traza; auditarla sería recursión, no información");
                });

            modelBuilder.Entity("Bastion.BuildingBlocks.Infrastructure.BandejaDeSalida.EventoDeLaBandeja", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<string>("Cuerpo")
                        .IsRequired()
                        .HasColumnType("jsonb")
                        .HasColumnName("cuerpo");

                    b.Property<Guid?>("EmpresaId")
                        .HasColumnType("uuid")
                        .HasColumnName("empresa_id");

                    b.Property<string>("Estado")
                        .IsRequired()
                        .HasMaxLength(32)
                        .HasColumnType("character varying(32)")
                        .HasColumnName("estado");

                    b.Property<Guid>("EventoId")
                        .HasColumnType("uuid")
                        .HasColumnName("evento_id");

                    b.Property<int>("Intentos")
                        .HasColumnType("integer")
                        .HasColumnName("intentos");

                    b.Property<string>("Nombre")
                        .IsRequired()
                        .HasMaxLength(128)
                        .HasColumnType("character varying(128)")
                        .HasColumnName("nombre");

                    b.Property<DateTimeOffset>("OcurridoEn")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("ocurrido_en");

                    b.Property<DateTimeOffset?>("PublicadoEn")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("publicado_en");

                    b.Property<string>("SinInquilino")
                        .HasMaxLength(64)
                        .HasColumnType("character varying(64)")
                        .HasColumnName("sin_inquilino");

                    b.Property<string>("UltimoError")
                        .HasMaxLength(1024)
                        .HasColumnType("character varying(1024)")
                        .HasColumnName("ultimo_error");

                    b.HasKey("Id")
                        .HasName("pk_bandeja_de_salida");

                    b.HasIndex("EventoId")
                        .IsUnique()
                        .HasDatabaseName("ix_bandeja_evento_id");

                    b.HasIndex("Estado", "Id")
                        .HasDatabaseName("ix_bandeja_de_salida_estado_id");

                    b.ToTable("bandeja_de_salida", "auditoria", t =>
                        {
                            t.ExcludeFromMigrations();

                            t.HasCheckConstraint("ck_bandeja_empresa_o_motivo", "(empresa_id IS NULL) <> (sin_inquilino IS NULL)");
                        });

                    b.HasAnnotation("Bastion:Auditoria", "NoAuditada|es la cola de eventos; publicarlos es fontanería, no un cambio de datos");
                });

            modelBuilder.Entity("Bastion.BuildingBlocks.Infrastructure.BandejaDeSalida.EventoProcesado", b =>
                {
                    b.Property<Guid>("EventoId")
                        .HasColumnType("uuid")
                        .HasColumnName("evento_id");

                    b.Property<string>("Consumidor")
                        .HasMaxLength(128)
                        .HasColumnType("character varying(128)")
                        .HasColumnName("consumidor");

                    b.Property<DateTimeOffset>("ProcesadoEn")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("procesado_en");

                    b.HasKey("EventoId", "Consumidor")
                        .HasName("pk_eventos_procesados");

                    b.ToTable("eventos_procesados", "auditoria", t =>
                        {
                            t.ExcludeFromMigrations();
                        });

                    b.HasAnnotation("Bastion:Auditoria", "NoAuditada|es la huella de que un consumidor ya pasó; no es un cambio de datos");
                });

            modelBuilder.Entity("Bastion.BuildingBlocks.Infrastructure.Idempotencia.RegistroDeIdempotencia", b =>
                {
                    b.Property<Guid>("EmpresaId")
                        .HasColumnType("uuid")
                        .HasColumnName("empresa_id");

                    b.Property<Guid>("UsuarioId")
                        .HasColumnType("uuid")
                        .HasColumnName("usuario_id");

                    b.Property<string>("Metodo")
                        .HasMaxLength(16)
                        .HasColumnType("character varying(16)")
                        .HasColumnName("metodo");

                    b.Property<string>("Ruta")
                        .HasMaxLength(512)
                        .HasColumnType("character varying(512)")
                        .HasColumnName("ruta");

                    b.Property<string>("Clave")
                        .HasMaxLength(128)
                        .HasColumnType("character varying(128)")
                        .HasColumnName("clave");

                    b.Property<int?>("CodigoDeEstado")
                        .HasColumnType("integer")
                        .HasColumnName("codigo_de_estado");

                    b.Property<DateTimeOffset>("CreadaEn")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("creada_en");

                    b.Property<string>("Cuerpo")
                        .HasColumnType("text")
                        .HasColumnName("cuerpo");

                    b.Property<string>("Etiqueta")
                        .HasMaxLength(128)
                        .HasColumnType("character varying(128)")
                        .HasColumnName("etiqueta");

                    b.Property<string>("Huella")
                        .IsRequired()
                        .HasMaxLength(64)
                        .HasColumnType("character varying(64)")
                        .HasColumnName("huella");

                    b.Property<string>("TipoDeContenido")
                        .HasMaxLength(128)
                        .HasColumnType("character varying(128)")
                        .HasColumnName("tipo_de_contenido");

                    b.Property<string>("Ubicacion")
                        .HasMaxLength(512)
                        .HasColumnType("character varying(512)")
                        .HasColumnName("ubicacion");

                    b.HasKey("EmpresaId", "UsuarioId", "Metodo", "Ruta", "Clave")
                        .HasName("pk_claves_de_idempotencia");

                    b.ToTable("claves_de_idempotencia", "auditoria", t =>
                        {
                            t.ExcludeFromMigrations();
                        });

                    b.HasAnnotation("Bastion:Auditoria", "NoAuditada|es el recibo de una petición ya atendida, no un cambio de datos");
                });

            modelBuilder.Entity("Bastion.Terceros.Domain.Terceros.CondicionPago", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<DateTimeOffset>("CreadoEn")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("creado_en")
                        .HasAnnotation("Bastion:Auditoria", "Auditada|");

                    b.Property<decimal?>("DescuentoPorProntoPago")
                        .HasPrecision(5, 2)
                        .HasColumnType("numeric(5,2)")
                        .HasColumnName("descuento_por_pronto_pago")
                        .HasAnnotation("Bastion:Auditoria", "Auditada|");

                    b.Property<int?>("DiaDePagoFijo")
                        .HasColumnType("integer")
                        .HasColumnName("dia_de_pago_fijo")
                        .HasAnnotation("Bastion:Auditoria", "Auditada|");

                    b.Property<int>("DiasDePlazo")
                        .HasColumnType("integer")
                        .HasColumnName("dias_de_plazo")
                        .HasAnnotation("Bastion:Auditoria", "Auditada|");

                    b.Property<DateTimeOffset>("ModificadoEn")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("modificado_en")
                        .HasAnnotation("Bastion:Auditoria", "NoAuditada|cambia en cada modificación y la traza ya lleva su propio `ocurrido_en`");

                    b.Property<string>("Rol")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)")
                        .HasColumnName("rol")
                        .HasAnnotation("Bastion:Auditoria", "Auditada|");

                    b.Property<Guid>("TerceroId")
                        .HasColumnType("uuid")
                        .HasColumnName("tercero_id")
                        .HasAnnotation("Bastion:Auditoria", "Auditada|");

                    b.HasKey("Id")
                        .HasName("pk_condiciones_pago");

                    b.HasIndex("TerceroId", "Rol")
                        .IsUnique()
                        .HasDatabaseName("ix_condiciones_pago_tercero_id_rol");

                    b.ToTable("condiciones_pago", "terceros", t =>
                        {
                            t.HasCheckConstraint("ck_condiciones_pago_plazo_legal", "dias_de_plazo BETWEEN 0 AND 60");
                        });

                    b.HasAnnotation("Bastion:Auditoria", "Auditada|");
                });

            modelBuilder.Entity("Bastion.Terceros.Domain.Terceros.Contacto", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<string>("Cargo")
                        .HasMaxLength(60)
                        .HasColumnType("character varying(60)")
                        .HasColumnName("cargo")
                        .HasAnnotation("Bastion:Auditoria", "Auditada|");

                    b.Property<string>("Correo")
                        .HasMaxLength(254)
                        .HasColumnType("character varying(254)")
                        .HasColumnName("correo")
                        .HasAnnotation("Bastion:Auditoria", "Auditada|");

                    b.Property<DateTimeOffset>("CreadoEn")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("creado_en")
                        .HasAnnotation("Bastion:Auditoria", "Auditada|");

                    b.Property<DateTimeOffset>("ModificadoEn")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("modificado_en")
                        .HasAnnotation("Bastion:Auditoria", "NoAuditada|cambia en cada modificación y la traza ya lleva su propio `ocurrido_en`");

                    b.Property<string>("Nombre")
                        .IsRequired()
                        .HasMaxLength(120)
                        .HasColumnType("character varying(120)")
                        .HasColumnName("nombre")
                        .HasAnnotation("Bastion:Auditoria", "Auditada|");

                    b.Property<string>("Telefono")
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)")
                        .HasColumnName("telefono")
                        .HasAnnotation("Bastion:Auditoria", "Auditada|");

                    b.Property<Guid>("TerceroId")
                        .HasColumnType("uuid")
                        .HasColumnName("tercero_id")
                        .HasAnnotation("Bastion:Auditoria", "Auditada|");

                    b.HasKey("Id")
                        .HasName("pk_contactos");

                    b.HasIndex("TerceroId")
                        .HasDatabaseName("ix_contactos_tercero_id");

                    b.ToTable("contactos", "terceros");

                    b.HasAnnotation("Bastion:Auditoria", "Auditada|");
                });

            modelBuilder.Entity("Bastion.Terceros.Domain.Terceros.CuentaBancaria", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<string>("Alias")
                        .HasMaxLength(60)
                        .HasColumnType("character varying(60)")
                        .HasColumnName("alias")
                        .HasAnnotation("Bastion:Auditoria", "Auditada|");

                    b.Property<string>("Bic")
                        .HasMaxLength(11)
                        .HasColumnType("character varying(11)")
                        .HasColumnName("bic")
                        .HasAnnotation("Bastion:Auditoria", "Auditada|");

                    b.Property<DateTimeOffset>("CreadoEn")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("creado_en")
                        .HasAnnotation("Bastion:Auditoria", "Auditada|");

                    b.Property<bool>("EsPreferente")
                        .HasColumnType("boolean")
                        .HasColumnName("es_preferente")
                        .HasAnnotation("Bastion:Auditoria", "Auditada|");

                    b.Property<string>("Iban")
                        .IsRequired()
                        .HasMaxLength(34)
                        .HasColumnType("character varying(34)")
                        .HasColumnName("iban")
                        .HasAnnotation("Bastion:Auditoria", "Auditada|");

                    b.Property<DateTimeOffset>("ModificadoEn")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("modificado_en")
                        .HasAnnotation("Bastion:Auditoria", "NoAuditada|cambia en cada modificación y la traza ya lleva su propio `ocurrido_en`");

                    b.Property<Guid>("TerceroId")
                        .HasColumnType("uuid")
                        .HasColumnName("tercero_id")
                        .HasAnnotation("Bastion:Auditoria", "Auditada|");

                    b.HasKey("Id")
                        .HasName("pk_cuentas_bancarias");

                    b.HasIndex("TerceroId")
                        .IsUnique()
                        .HasDatabaseName("ix_cuentas_bancarias_una_preferente_por_tercero")
                        .HasFilter("es_preferente");

                    b.HasIndex("TerceroId", "Iban")
                        .IsUnique()
                        .HasDatabaseName("ix_cuentas_bancarias_tercero_id_iban");

                    b.ToTable("cuentas_bancarias", "terceros");

                    b.HasAnnotation("Bastion:Auditoria", "Auditada|");
                });

            modelBuilder.Entity("Bastion.Terceros.Domain.Terceros.Tercero", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnType("uuid")
                        .HasColumnName("id");

                    b.Property<DateTimeOffset>("CreadoEn")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("creado_en")
                        .HasAnnotation("Bastion:Auditoria", "Auditada|");

                    b.Property<Guid>("EmpresaId")
                        .HasColumnType("uuid")
                        .HasColumnName("empresa_id")
                        .HasAnnotation("Bastion:Auditoria", "Auditada|");

                    b.Property<bool>("EsCliente")
                        .HasColumnType("boolean")
                        .HasColumnName("es_cliente")
                        .HasAnnotation("Bastion:Auditoria", "Auditada|");

                    b.Property<bool>("EsProveedor")
                        .HasColumnType("boolean")
                        .HasColumnName("es_proveedor")
                        .HasAnnotation("Bastion:Auditoria", "Auditada|");

                    b.Property<DateTimeOffset>("ModificadoEn")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("modificado_en")
                        .HasAnnotation("Bastion:Auditoria", "NoAuditada|cambia en cada modificación y la traza ya lleva su propio `ocurrido_en`");

                    b.Property<string>("NombreComercial")
                        .HasColumnType("text")
                        .HasColumnName("nombre_comercial")
                        .HasAnnotation("Bastion:Auditoria", "Auditada|");

                    b.Property<string>("RazonSocial")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("razon_social")
                        .HasAnnotation("Bastion:Auditoria", "Auditada|");

                    b.Property<Guid?>("TarifaAsignadaId")
                        .HasColumnType("uuid")
                        .HasColumnName("tarifa_asignada_id")
                        .HasAnnotation("Bastion:Auditoria", "Auditada|");

                    b.Property<uint>("Version")
                        .IsConcurrencyToken()
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("xid")
                        .HasColumnName("xmin")
                        .HasAnnotation("Bastion:Auditoria", "NoAuditada|es el testigo de concurrencia (xmin): lo pone PostgreSQL en cada escritura y no es un dato del negocio. Auditar su cambio sería anotar que hubo un cambio en la misma fila que ya anota cuál fue.");

                    b.ComplexProperty(typeof(Dictionary<string, object>), "Bloqueo", "Bastion.Terceros.Domain.Terceros.Tercero.Bloqueo#Bloqueo", b1 =>
                        {
                            b1.IsRequired();

                            b1.Property<DateTimeOffset?>("Desde")
                                .HasColumnType("timestamp with time zone")
                                .HasColumnName("bloqueado_en")
                                .HasAnnotation("Bastion:Auditoria", "Auditada|");

                            b1.Property<bool>("EstaBloqueado")
                                .HasColumnType("boolean")
                                .HasColumnName("bloqueado")
                                .HasAnnotation("Bastion:Auditoria", "Auditada|");

                            b1.Property<string>("Motivo")
                                .HasColumnType("text")
                                .HasColumnName("motivo_del_bloqueo")
                                .HasAnnotation("Bastion:Auditoria", "Auditada|");
                        });

                    b.ComplexProperty(typeof(Dictionary<string, object>), "DomicilioFiscal", "Bastion.Terceros.Domain.Terceros.Tercero.DomicilioFiscal#Direccion", b1 =>
                        {
                            b1.IsRequired();

                            b1.Property<string>("Calle")
                                .IsRequired()
                                .HasMaxLength(70)
                                .HasColumnType("character varying(70)")
                                .HasColumnName("domicilio_fiscal_calle")
                                .HasAnnotation("Bastion:Auditoria", "Auditada|");

                            b1.Property<string>("CodigoPostal")
                                .IsRequired()
                                .HasMaxLength(16)
                                .HasColumnType("character varying(16)")
                                .HasColumnName("domicilio_fiscal_codigo_postal")
                                .HasAnnotation("Bastion:Auditoria", "Auditada|");

                            b1.Property<string>("Numero")
                                .HasMaxLength(16)
                                .HasColumnType("character varying(16)")
                                .HasColumnName("domicilio_fiscal_numero")
                                .HasAnnotation("Bastion:Auditoria", "Auditada|");

                            b1.Property<string>("Pais")
                                .IsRequired()
                                .HasMaxLength(2)
                                .HasColumnType("character varying(2)")
                                .HasColumnName("domicilio_fiscal_pais")
                                .HasAnnotation("Bastion:Auditoria", "Auditada|");

                            b1.Property<string>("Poblacion")
                                .IsRequired()
                                .HasMaxLength(35)
                                .HasColumnType("character varying(35)")
                                .HasColumnName("domicilio_fiscal_poblacion")
                                .HasAnnotation("Bastion:Auditoria", "Auditada|");

                            b1.Property<string>("Subdivision")
                                .HasMaxLength(35)
                                .HasColumnType("character varying(35)")
                                .HasColumnName("domicilio_fiscal_subdivision")
                                .HasAnnotation("Bastion:Auditoria", "Auditada|");
                        });

                    b.ComplexProperty(typeof(Dictionary<string, object>), "Identificacion", "Bastion.Terceros.Domain.Terceros.Tercero.Identificacion#IdentificacionFiscal", b1 =>
                        {
                            b1.IsRequired();

                            b1.Property<string>("Numero")
                                .IsRequired()
                                .HasMaxLength(20)
                                .HasColumnType("character varying(20)")
                                .HasColumnName("identificacion_numero")
                                .HasAnnotation("Bastion:Auditoria", "Auditada|");

                            b1.Property<string>("Pais")
                                .IsRequired()
                                .HasMaxLength(2)
                                .HasColumnType("character varying(2)")
                                .HasColumnName("identificacion_pais")
                                .HasAnnotation("Bastion:Auditoria", "Auditada|");

                            b1.Property<string>("Verificacion")
                                .IsRequired()
                                .HasColumnType("text")
                                .HasColumnName("identificacion_verificacion")
                                .HasAnnotation("Bastion:Auditoria", "Auditada|");
                        });

                    b.ComplexProperty(typeof(Dictionary<string, object>), "LimiteCredito", "Bastion.Terceros.Domain.Terceros.Tercero.LimiteCredito#Importe", b1 =>
                        {
                            b1.Property<decimal>("Cantidad")
                                .HasPrecision(18, 4)
                                .HasColumnType("numeric(18,4)")
                                .HasColumnName("limite_credito_cantidad")
                                .HasAnnotation("Bastion:Auditoria", "Auditada|");

                            b1.Property<string>("Divisa")
                                .IsRequired()
                                .HasMaxLength(3)
                                .HasColumnType("character varying(3)")
                                .HasColumnName("limite_credito_divisa")
                                .HasAnnotation("Bastion:Auditoria", "Auditada|");
                        });

                    b.ComplexProperty(typeof(Dictionary<string, object>), "RegimenFiscal", "Bastion.Terceros.Domain.Terceros.Tercero.RegimenFiscal#RegimenFiscal", b1 =>
                        {
                            b1.IsRequired();

                            b1.Property<bool>("CriterioDeCaja")
                                .HasColumnType("boolean")
                                .HasColumnName("criterio_de_caja")
                                .HasAnnotation("Bastion:Auditoria", "Auditada|");

                            b1.Property<bool>("RecargoDeEquivalencia")
                                .HasColumnType("boolean")
                                .HasColumnName("recargo_de_equivalencia")
                                .HasAnnotation("Bastion:Auditoria", "Auditada|");

                            b1.Property<bool>("SujetoARetencionIrpf")
                                .HasColumnType("boolean")
                                .HasColumnName("sujeto_a_retencion_irpf")
                                .HasAnnotation("Bastion:Auditoria", "Auditada|");

                            b1.Property<string>("Territorio")
                                .IsRequired()
                                .HasMaxLength(20)
                                .HasColumnType("character varying(20)")
                                .HasColumnName("territorio_fiscal")
                                .HasAnnotation("Bastion:Auditoria", "Auditada|");
                        });

                    b.HasKey("Id")
                        .HasName("pk_terceros");

                    b.ToTable("terceros", "terceros", t =>
                        {
                            t.HasCheckConstraint("ck_terceros_limite_credito_completo", "(limite_credito_cantidad IS NULL) = (limite_credito_divisa IS NULL)");

                            t.HasCheckConstraint("ck_terceros_territorio_fiscal", "territorio_fiscal IN ('PeninsulaYBaleares', 'Canarias', 'CeutaYMelilla', 'UnionEuropea', 'TercerosPaises')");
                        });

                    b.HasAnnotation("Bastion:Auditoria", "Auditada|");
                });

            modelBuilder.Entity("Bastion.Terceros.Domain.Terceros.CondicionPago", b =>
                {
                    b.HasOne("Bastion.Terceros.Domain.Terceros.Tercero", null)
                        .WithMany("CondicionesPago")
                        .HasForeignKey("TerceroId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_condiciones_pago_terceros_tercero_id");
                });

            modelBuilder.Entity("Bastion.Terceros.Domain.Terceros.Contacto", b =>
                {
                    b.HasOne("Bastion.Terceros.Domain.Terceros.Tercero", null)
                        .WithMany("Contactos")
                        .HasForeignKey("TerceroId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_contactos_terceros_tercero_id");
                });

            modelBuilder.Entity("Bastion.Terceros.Domain.Terceros.CuentaBancaria", b =>
                {
                    b.HasOne("Bastion.Terceros.Domain.Terceros.Tercero", null)
                        .WithMany("CuentasBancarias")
                        .HasForeignKey("TerceroId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_cuentas_bancarias_terceros_tercero_id");
                });

            modelBuilder.Entity("Bastion.Terceros.Domain.Terceros.Tercero", b =>
                {
                    b.Navigation("CondicionesPago");

                    b.Navigation("Contactos");

                    b.Navigation("CuentasBancarias");
                });
#pragma warning restore 612, 618
        }
    }
}
