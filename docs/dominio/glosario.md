...
| --- | --- | --- | --- | --- |
| Ajuste de inventario | `Ajuste` | Inventario | Empresa | El **documento** con el que una persona corrige las existencias y explica por qué: empresa, almacén, fecha de operación, motivo y sus líneas. Es el primero de la familia de documentos del §7.4 y el primero que escribe en el libro (R13). Tiene **máquina de estados** sobre el tipo base del bloque común —`Borrador`, `Confirmado`, `Anulado`—, y el estado no se asigna: se transita, porque `documento.Estado = X` no compila. **No es el movimiento**: confirmarlo es lo que genera las filas del libro, en la misma transacción, y antes de eso no hay ni una. **Y no se borra ni se corrige hacia atrás**: un ajuste confirmado se anula con uno inverso, porque lo que ya movió el stock no se puede hacer como si no hubiera pasado. **Sí se audita**, al revés que el libro: aquí hay decisiones de una persona, y una traza de sus cambios contesta preguntas que la fila del libro no contesta.
...
| Artículo | `Articulo` | Catálogo | Empresa | Lo que se compra, se vende o se fabrica: código, descripción, tipo (`Bien` o `Servicio`), **unidad base** e **impuesto por defecto**. Los dos últimos son identificadores de Organización **validados por su puerto y no claves ajenas**: viven en otro esquema (R4). El código y la unidad base **no se editan** —reexpresar un histórico entero en otra unidad no es una edición, es otro artículo—. **No es la existencia**: cuánto hay y dónde está es la fase 2. **Tampoco es el precio**, que es la tarifa del 1.9. **No se bloquea**, y eso está comprobado y no supuesto: no hay un solo dato de persona en la ficha, así que el artículo 32 no tiene aquí nada que reservar. |
| Suministro | `ArticuloProveedor` | Catálogo | Empresa | Que **un tercero suministra un artículo**, y con qué referencia lo llama él. Es un hecho **entre dos** y no un atributo del artículo: por eso es agregado propio y por eso se borra de verdad cuando deja de ser verdad. El tercero es un identificador de Terceros **validado por su puerto y no clave ajena** —vive en otro esquema (R4)—, y el puerto no contesta «¿existe?» sino en qué estado está y si hace de proveedor. **No es un precio de compra**, que es la fase 3; **no es una existencia**, que es la fase 2. Y es **la razón por la que Catálogo participa en el artículo 32 sin guardar un solo dato de nadie**: no guarda un nombre, pero señala a una ficha que puede ser la de una persona física, así que el listado de proveedores de un artículo se filtra preguntando por el bloqueo del tercero al que apunta. |
| Categoría | `Categoria` | Catálogo | Empresa | La rama del árbol con el que **una empresa** clasifica su catálogo: código, nombre y de qué categoría cuelga —ninguna, si es raíz—. Está modelada como **lista de adyacencia** y no como tabla de cierre, con su motivo en `docs/PLAN.md`. **No es una etiqueta**: un artículo cuelga como mucho de una. **Y no es un atributo del artículo disfrazado de rama**, que es lo que suele haber a partir del décimo nivel — de ahí que el árbol tenga cota, y que colgar de más allá se rechace. |
| Condición de pago | `CondicionPago` | Terceros | Empresa | En cuántos días paga —o se le paga a— un tercero, con su día fijo y su descuento por pronto pago. **Guarda el plazo, no un vencimiento**: los días se cuentan desde la **entrega o la prestación** (art. 4 de la Ley 3/2004) y Terceros no ve entregas. El tope de sesenta días es un **invariante**, no un valor por omisión: no es ampliable por acuerdo, así que un plazo mayor no es una condición peor, es una cláusula nula. Hay **una por papel**: lo que se concede cobrando no es lo que se acepta pagando. |
| Contacto | `Contacto` | Terceros | Empresa | Una persona con la que se habla en casa del tercero: nombre, cargo, correo y teléfono **profesionales**. **No es un usuario** —no entra en el sistema— ni el interesado de la ficha. **No tiene campo de notas**, y la ausencia es la decisión: un `text` libre acaba guardando lo que no se declaró recoger. Se borra de verdad, porque nada de lo emitido cuelga de él. |
| Cuenta bancaria | `CuentaBancaria` | Terceros | Empresa | Por dónde se le paga a un tercero o se le cobra: IBAN validado con mod-97-10 y la longitud de su país, más un BIC opcional del que solo se comprueba la forma. **Como mucho una preferente** por ficha, sostenida por el agregado y por un índice único parcial. El IBAN **no se cambia**: una cuenta distinta es otra cuenta. |
...
| Línea de ajuste | `LineaDeAjuste` | Inventario | Empresa | Lo que el ajuste dice de **un** artículo en **una** ubicación: la cantidad tal como se escribió, la unidad en la que se escribió, el factor a la unidad base y el coste unitario. Es parte del agregado del ajuste —se carga y se guarda con él— y por eso **no filtra por empresa** ella sola: filtra el documento del que cuelga, y que no se consulte suelta está comprobado por nombre. El almacén está en la **cabecera** y la ubicación en la **línea**, así que un ajuste mueve varias estanterías del mismo almacén de una vez. **No es el movimiento**: es lo que se quiere mover; el movimiento es la fila que queda escrita cuando el documento se confirma.
| Línea de tarifa | `LineaTarifa` | Catálogo | Empresa | Lo que una tarifa dice de **un** destino a partir de **una** cantidad: o un precio, o un descuento en tanto por ciento, **nunca los dos y nunca ninguno**. El destino es un artículo **o** una categoría, con claves ajenas de verdad —son del mismo esquema (R4)— y exclusivas entre sí. **No es «el precio del artículo»**: varias líneas del mismo destino forman una escala por cantidad, y la que se aplica es la del tramo en el que cae la cantidad pedida. El primer tramo de cada destino empieza en cero, que es lo que impide un hueco en medio de una tabla de precios escrita. |
| Movimiento de existencias | `MovimientoStock` | Inventario | Empresa | El **hecho** de que una cantidad de un artículo entró o salió de una ubicación un día, con su coste unitario y el documento que lo provocó (R3). La cantidad va **con signo y en la unidad base**, y al lado la cantidad y la unidad **tal como se introdujeron** más el factor que las une: sin las tres, «2,5 cajas» deja de poder reimprimirse en cuanto alguien corrige la conversión. Su fecha de operación es una **fecha de negocio** (R14) y además la clave por la que se particiona la tabla. **No es el saldo**: cuántas hay es la SUMA de estas filas y no se guarda en ninguna columna, que es lo que impide que existan dos verdades sobre las mismas existencias —el saldo es el 2.7—. **No se edita ni se borra**, y eso no lo sostiene una buena costumbre: la tabla rechaza `UPDATE`, `DELETE` y `TRUNCATE` con disparadores, también sobre cada partición. **No se audita**, y por eso mismo: una traza de cambios sobre lo que no cambia registraría cero filas pesando lo que la tabla; lo que se audita es el documento. **Y no se bloquea** (R16): bloquear un hecho contable sería esconderlo, y que el almacén al que apunta esté bloqueado no borra lo que ya pasó. El documento de origen viaja como **tipo más identificador**, no como clave ajena: apunta a tablas distintas según el tipo. |
...
| Tarifa | `Tarifa` | Catálogo | Empresa | Una lista de precios de una empresa durante un intervalo de días, en **una divisa**: código, nombre, divisa y vigencia. **No es «los precios»** —eso son sus líneas— y **no es única por código**: `PVP` tiene tantas filas como veces cambió la lista, como el impuesto. Una tarifa **no se edita para subir precios**: se **cierra** el tramo vigente y se abre el siguiente, para que reimprimir un albarán de hace un año siga dando el importe de entonces. Dos tramos del mismo código **no pueden pisarse ni un día**, y eso lo sostiene una restricción de exclusión de la base sobre un `daterange` cerrado por los dos extremos, no una comprobación previa. Su divisa **puede no ser la de la empresa** y se acepta a propósito; convertir no es de este ítem, y por eso el precio resuelto viaja **siempre** con su divisa pegada. |
...
| **Régimen fiscal** | Objeto de valor con el **territorio** —península y Baleares, Canarias, Ceuta y Melilla, Unión Europea o terceros países—, y tres marcas: recargo de equivalencia, criterio de caja y sujeción a retención de IRPF. | **No es el tipo de retención.** El porcentaje tiene dueño desde el 1.2 —`Impuesto` con su vigencia— y además varía por tercero; ponerlo aquí sería una segunda fuente para el mismo número. Este campo dice **si** procede, no **cuánto**. |
| **Límite de crédito** | Cuánto se le fía, como `Importe` —o sea, **con su divisa**— o nada si no se le controla el crédito. | **No es un saldo.** Consumido, disponible y bloqueo por exceso necesitan ver pedidos y facturas pendientes: eso es la fase 6. Y **no hereda la divisa de la empresa**: un importe que no dice de qué es sería el único del sistema que no lo dice. |
