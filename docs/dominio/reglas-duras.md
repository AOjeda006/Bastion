...
| R5 | La numeración de documentos es legal: por serie y ejercicio, correlativa y sin huecos. | aplazada a la fase 2 | La `Serie` existe con su contador en una columna (ADR-0007), pero sin bloqueo en la base, y `Serie.RegistrarNumeroAsignado` solo lo llaman los tests. **Lo que el ítem 2.3 deja viejo del motivo anterior es su final, «nada confirma todavía un documento»**: el `Ajuste` se confirma desde el 2.3, y se confirma **sin número**, que es justo lo que la regla no permite. El cerrojo sobre la fila de la serie y el número al confirmar son el ítem **2.4**; la numeración sin huecos bajo concurrencia sigue siendo criterio de la fase 5. |
